...
private static final double NOSE_HEIGHT =0.1;
...
nose = createNose(NOSE_WIDTH * width, NOSE_HEIGHT * mouth = new GRect(MOUTH_WIDTH * width, MOUTH_HEIGHT);
add(head, 0, 0);
add(leftEye, (0.25 * width) - (EYE_WIDTH * width) /height);* height);
2,
(0.25 * height) - (EYE_HEIGHT * height) / 2); add(rightEye, (0.75 * width) - (EYE_WIDTH * width) / 2,
(0.25 * height) - (EYE_HEIGHT * height) / 2); add(nose, 0.50 * width, 0.50 * height);
